function f = alligned_heatmap_view (folder)
% Returns a heat-map of all neural activity in a single stage. Every line
% is the average activity of a single neuron during the entire day - averaged over different trials. 
...
% each cell.
%
% Syntax:  alligned_heatmap_view ('C:\Users\GEORGEKOUR\Desktop\Rat10_data\odor1_WR')
%
% Inputs:
%    stage_folder - The folder of the stage

[median_times, proportions, num_bins_per_proportion, num_bins_between_events] = analyze_behavior_in_stage(folder); %Makes the analysis per day

SS_files = dir([folder,'\*_SS_*.ntt']); %look for all single units files in the stage
bined_spikes_all_SS = [];
spike_counts = [];
